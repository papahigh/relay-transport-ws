const _global = typeof global !== 'undefined' ? global : typeof window !== 'undefined' ? window : ({} as any)
...
export class SubscriptionClient {
  private static checkOperationOptions(options: OperationOptions, handler: (error: Error[], result?: any) => void) {
  ...
  private static buildMessage(id: string | undefined, type: string, payload: any) {
  ...
  private readonly connectionParams: Function
  ...
  public request(request: OperationOptions): Observable<ExecutionResult> {
    ...
    return {
      ...
      subscribe(
        ...
        opId = executeOperation(request, (error: Error[] | null, result: any) => {
  ...
  private executeOperation(options: OperationOptions, handler: (error: Error[] | null, result?: any) => void): string {
    if (this.client === null) {
      this.connect()
    }
    ...
    this.applyMiddlewares(options)
      .then(processedOptions => {
        SubscriptionClient.checkOperationOptions(processedOptions, handler)
  ...
  private sendMessage(id: string | undefined, type: string, payload: any) {
    this.sendMessageRaw(SubscriptionClient.buildMessage(id, type, payload))
  }
    ...
    if (!this.reconnecting) {
      Object.keys(this.operations).forEach(key => {
        this.unsentMessagesQueue.push(
          SubscriptionClient.buildMessage(key, MessageTypes.GQL_START, this.operations[key].options),
        )
      })
